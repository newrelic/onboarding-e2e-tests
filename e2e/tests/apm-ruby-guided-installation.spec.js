...
test.skip("should shows steps to install the Ruby", async () => {
  test.slow();
  ...
  await expect(warningText).toContainText("You must complete steps  1, and 2");
  ...
  const noBundler = await page.locator('div[data-test-id="setup.no-bundler"]');
