...
test.describe('Node JS Guided installation', () => {
  test('should shows different methods to install the Node JS', async () => {
    ...
    await page.getByRole('button', { name: 'Select your language (Node JS)' }).isVisible();
    ...
    await expect(page.getByText('Choose your instrumentation method')).toBeVisible();
    ...
    await expect(page.getByText('On a host (PM2)', { exact: true })).toBeVisible();

    await expect(page.getByText('On a host (without PM2)', { exact: true })).toBeVisible();
  ...
  test('should shows steps to install the Node JS through Package manager', async () => {
    ...
    await expect(
      page.getByText(`Download the config file and put it in your Node.js application’s root directory.`)
    ).toBeVisible();

    await expect(page.getByText(` * New Relic agent configuration.`)).toBeVisible();
    ...
    await expect(page.getByText(`node -r newrelic YOUR_MAIN_FILENAME.js`)).toBeVisible();
    ...
    await expect(
      page.getByText(`node --experimental-loader=newrelic/esm-loader.mjs YOUR_MAIN_FILENAME.js`)
    ).toBeVisible();

    await page.getByRole('button', { name: 'Continue' }).click();

    await expect(page.getByText('Connect your logs and infrastructure')).toBeVisible();
    ...
    await expect(page.getByText('Restart your app, then test your connection.')).toBeVisible();

    await expect(page.getByRole('button', { name: 'Test connection' })).toBeVisible();

    await expect(page.getByRole('button', { name: 'See your data' })).toBeDisabled();
  ...
  test('should shows steps to install the Node JS through Docker', async () => {
    ...
    await expect(page.getByText(`Add 'newrelic' as a dependency to your package.json file`)).toBeVisible();

    await expect(page.getByText(`"newrelic": "latest"`)).toBeVisible();

    await expect(page.getByText(`In the first line of your app's main module, add:`)).toBeVisible();

    await expect(page.getByText(`require('newrelic');`)).toBeVisible();

    await expect(page.getByText(`Add this ENV line to your Dockerfile`)).toBeVisible();

    await expect(page.getByText(`ENV NEW_RELIC_NO_CONFIG_FILE=true`)).toBeVisible();

    await expect(page.getByText(`Set the config options via ENV directives`)).toBeVisible();

    await expect(page.getByText('ENV NEW_RELIC_DISTRIBUTED_TRACING_ENABLED=true')).toBeVisible();

    const [otherConfigOptionsDoc] = await Promise.all([
      page.waitForEvent('popup'),
      page.getByRole('link', { name: 'other agent configuration options' }).click(),
    ]);

    await otherConfigOptionsDoc.getByRole('heading', { name: 'Node.js agent configuration' }).isVisible();

    await otherConfigOptionsDoc.close();

    await expect(page.getByText('Add environment variables and run the command')).toBeVisible();
    ...
    await nodeJSReleaseNotesDoc.getByRole('heading', { name: 'Node.js agent release notes' }).isVisible();
    ...
    await packageJsonDoc.getByRole('heading', { name: 'package.json' }).isVisible();
  ...
  test('should shows steps to install the Node JS On a host (PM2)', async () => {
    ...
    await expect(page.getByText('If you use a firewall, configure your proxy')).toBeVisible();

    await expect(page.getByText('Run this command on your host to download and install the integration')).toBeVisible();

    await expect(
      page.getByText('curl -Ls https://download.newrelic.com/install/newrelic-cli/scripts/install.sh')
    ).toBeVisible();
    ...
    await installationDocs.getByRole('heading', { name: 'Install the Node.js agent' }).isVisible();
  ...
  test('should shows steps to install the Node JS On a host (without PM2)', async () => {
    ...
    await page.getByRole('heading', { name: 'On a host (without PM2)' }).click();
