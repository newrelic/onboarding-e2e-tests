...
test('should shows steps to install the Go agent', async () => {
  ...
  await page.getByRole('button', { name: 'Select your language (Go)' }).isVisible();
  ...
  await expect(page.getByText('Run this command to get the Go agent:')).toBeVisible();

  await expect(page.getByText(`go get github.com/newrelic/go-agent/v3`)).toBeVisible();

  await expect(
    page.getByText(`In your application, import the package github.com/newrelic/go-agent/v3/newrelic.`)
  ).toBeVisible();

  await expect(page.getByText(`Add this code to your main function or init block:`)).toBeVisible();

  /* NEED TO ADD data-test-id TO VALIDATE THE GO AGENT INITIALIZATION COMMAND */

  await expect(
    page.getByText(`To monitor web transactions, in your app code wrap standard HTTP requests. For example:`)
  ).toBeVisible();

  await expect(page.getByText(`http.HandleFunc(newrelic.WrapHandleFunc(app, "/users", usersHandler))`)).toBeVisible();
  ...
  await page.getByRole('heading', { name: 'Install New Relic for Go' }).isVisible();
  ...
  await page.getByRole('heading', { name: 'Go agent configuration' }).isVisible();
  ...
  await page.getByRole('heading', { name: 'Go agent logs in context' }).isVisible();
  ...
  await page.getByRole('heading', { name: 'Go agent compatibility and requirements' }).isVisible();
  ...
  await page.getByRole('heading', { name: 'Do you have specific feedback for us?' }).click();
